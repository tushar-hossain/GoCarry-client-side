...
function Coverage() {
  ...
  const filteredDistricts = useMemo(() => {
    ...
    return warehouses
      ?.filter((warehouse) =>
        warehouse?.district?.toLowerCase()?.includes(value),
      )
      ?.slice(0, 6);
  }, [search, warehouses]);
  ...
  const handleSearch = () => {
    ...
    const district =
      warehouses?.find((item) => item.district?.toLowerCase() === value) ||
      warehouses?.find((item) =>
        item.district?.toLowerCase().startsWith(value),
      ) ||
      warehouses?.find((item) => item.district?.toLowerCase()?.includes(value));
  ...
  return (
    <section className="md:max-w-6xl mx-auto bg-[#eef0f1] px-3 py-8 sm:px-5 sm:py-10 lg:px-0 lg:py-5">
      <div className="mx-auto w-full rounded-[14px] bg-white px-5 py-4 sm:px-8 sm:py-10 lg:px-13 lg:py-9.5">
        <div>
          <h2 className="text-[25px] font-bold leading-8 tracking-[-0.7px] text-[#03373D] sm:text-[30px] sm:leading-9.5 lg:text-[28px]">
            We are available in 64 districts
          </h2>
        </div>
        <div className="relative mt-6 w-full max-w-68.75">
          <div className="flex h-8.5 overflow-hidden rounded-full bg-[#eef1f3]">
            <div className="flex w-8 shrink-0 items-center justify-center">
              <Search className="h-3.25 w-3.25 text-[#03373D]" />
            </div>
            ...
            <button
              ...
              className="flex h-8.25 w-15.5 shrink-0 cursor-pointer items-center justify-center rounded-full bg-[#CAEB66] text-[10px] font-semibold text-[#03373D] transition hover:brightness-95"
            >
              Search
            </button>
          </div>
          ...
          {showSuggestions && search && filteredDistricts?.length > 0 && (
            <div className="absolute left-0 right-0 top-10 z-1000 overflow-hidden rounded-xl border border-slate-100 bg-white shadow-lg">
              {filteredDistricts?.map((warehouse) => (
                <button
                  key={warehouse._id}
                  type="button"
                  onClick={() => handleSelectDistrict(warehouse)}
        ...
        <div className="mt-6 h-75 w-full overflow-hidden sm:h-87.5 lg:h-90">
          <MapContainer
            ...
            {warehouses?.map((warehouse) => (
              <Marker
                ...
                icon={createMarkerIcon(
                  selectedDistrict?.district === warehouse.district,
                )}
                eventHandlers={{
                  click: () => {
                    setSelectedDistrict(warehouse);
                    setSearch(warehouse.district);
                  },
                }}
              />
            ))}
